...
export default function Tree({ }: TreeProps): JSX.Element {
    ...
    const renderNode = (rootNode: INode) => {
        ...
        return (
            <li className="tree__item">
                <div className={`tree__text ${activeTreeItemId === rootNode.nodeID ? 'active' : ''}`} onClick={() => openSettings(rootNode.nodeID)}>{rootNode.name}</div>
    ...
    return (
        <ul className="tree">{renderNode(rootNode)}</ul>
    );
}
